...
public class Market {
    ...
    public static HashMap<String, ArrayList<Integer>> buyAction(HashMap<Items, Integer> purchaseTable, HashMap<String, ArrayList<Integer>> playerCargo) {
        ...
        for (int i = 0; i < items.length; i++) {
            if (purchaseTable.get(items[i]) != null) {
                ...
                quantiPrice.add(0, purchaseTable.get(items[i])+ playerCargo.get(items[i].getName()).get(0));
    ...
    public static HashMap<String, ArrayList<Integer>> buyAction2(HashMap<Items, Integer> purchaseTable, HashMap<String, ArrayList<Integer>> planetCargo) {
        Items[] items = Items.values();
        HashMap<String, ArrayList<Integer>> newCargo = new HashMap<>();
        ...
        for (int i = 0; i < items.length; i++) {
            if (purchaseTable.get(items[i]) != null) {
                ArrayList<Integer> quantiPrice = new ArrayList<>();
                quantiPrice.add(0,planetCargo.get(items[i].getName()).get(0) - purchaseTable.get(items[i]));
